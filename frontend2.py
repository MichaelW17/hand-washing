...
class FrontEnd(QMainWindow):
    ...
    def initUI(self):
        ...
        self.bgColor.setColor(QPalette.Background, QColor(255, 255, 255))
        # self.win.bgColor.setColor(self.backgroundRole(), QColor(255, 255, 255))
        self.win.setAutoFillBackground(True)
        self.win.setPalette(self.bgColor)
    ...
    def restart(self):  # 当前视频播放完毕，重播一遍
        ...
        if self.time < 1000:  # 洗手时间不足20s
            self.bgColor.setColor(QPalette.Background, QColor(240, 128, 128))
        else:
            self.bgColor.setColor(QPalette.Background, QColor(135, 206, 250))
        self.win.setPalette(self.bgColor)
        self.repaint()
